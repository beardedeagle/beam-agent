...
find_text_matches_pattern_test() ->
    ...
    {ok, Results} = beam_agent_catalog:file_find_text(<<"beta">>, #{cwd => Dir}),
...
find_text_no_match_returns_empty_test() ->
    ...
    {ok, Results} = beam_agent_catalog:file_find_text(<<"zzzzz">>, #{cwd => Dir}),
    ?assertEqual([], Results),
    cleanup_tmp().

find_text_invalid_regex_returns_error_test() ->
    {error, {invalid_pattern, _Reason, _Pos}} =
        beam_agent_catalog:file_find_text(<<"[invalid">>, #{}).
...
find_text_with_glob_test() ->
    ...
    {ok, Results} = beam_agent_catalog:file_find_text(
        <<"-module">>, <<"*.erl">>, #{cwd => Dir}),
...
find_files_returns_entries_test() ->
    ...
    {ok, Entries} = beam_agent_catalog:file_find_files(#{cwd => Dir}),
...
find_files_with_glob_test() ->
    ...
    {ok, Entries} = beam_agent_catalog:file_find_files(<<"*.erl">>, #{cwd => Dir}),
...
find_symbols_erlang_function_test() ->
    ...
    write_tmp_file(Dir, <<"mod.erl">>,
        <<"-module(mod).\nmy_func(X) -> X.\n">>),
    {ok, Results} = beam_agent_catalog:file_find_symbols(<<"my_func">>, #{cwd => Dir}),
    ?assert(length(Results) >= 1),
    cleanup_tmp().
...
find_symbols_no_query_test() ->
    ...
    write_tmp_file(Dir, <<"defs.erl">>,
        <<"-spec start() -> ok.\nstart() -> ok.\n">>),
    {ok, Results} = beam_agent_catalog:file_find_symbols(#{cwd => Dir}),
    ?assert(length(Results) >= 1),
    cleanup_tmp().
...
file_list_valid_directory_test() ->
    ...
    {ok, Entries} = beam_agent_catalog:file_list_impl(Dir),
...
file_list_invalid_directory_test() ->
    {error, {list_dir_failed, _, _}} =
        beam_agent_catalog:file_list_impl(<<"/tmp/beam_agent_no_such_dir_xyzzy">>).
...
file_read_existing_file_test() ->
    ...
    {ok, Result} = beam_agent_catalog:file_read_impl(Path),
...
file_read_missing_file_test() ->
    {error, {read_failed, _, _}} =
        beam_agent_catalog:file_read_impl(<<"/tmp/beam_agent_no_such_file_xyzzy.txt">>).
...
file_status_returns_ok_test() ->
    {ok, Status} = beam_agent_catalog:file_status_impl(),
...
file_status_with_cwd_opt_test() ->
    {ok, Status} = beam_agent_catalog:file_status_impl(#{cwd => <<"/tmp">>}),
    ?assert(maps:is_key(cwd, Status)),
    ?assert(maps:is_key(source, Status)),
    ?assert(maps:is_key(files, Status)).
