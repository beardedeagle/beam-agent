...
universal_review_and_config_fallbacks_test() ->
    ...
    {ok, Modes} = beam_agent_control:collaboration_mode_list(Session),
    ...
    {ok, _Config} = beam_agent_config:update(Session, #{
        provider_id => <<"openai">>,
    ...
    {ok, Methods} = beam_agent_provider:auth_methods(Session),
...
universal_realtime_fallback_records_thread_history_test() ->
    ...
    {ok, Features} = beam_agent_control:experimental_feature_list(Session),
    ?assertEqual(universal, maps:get(source, Features)),
    {ok, TextSession} = beam_agent_control:thread_realtime_append_text(Session, ThreadId, #{
        text => <<"hello realtime">>
    }),
    ?assertEqual(<<"hello realtime">>, maps:get(last_text, TextSession)),
    ?assertEqual(1, length(maps:get(inputs, TextSession))),
    {ok, AudioSession} = beam_agent_control:thread_realtime_append_audio(Session, ThreadId, #{
        mime => <<"audio/wav">>,
    ...
    {ok, #{messages := Messages}} =
        beam_agent_threads:thread_read(Session, ThreadId, #{include_messages => true}),
    ...
    {ok, Stopped} = beam_agent_control:thread_realtime_stop(Session, ThreadId),
...
universal_review_and_realtime_cover_non_native_backends_test() ->
    lists:foreach(fun(Backend) ->
        ...
        {ok, Updated} = beam_agent_control:thread_realtime_append_text(Session, ThreadId, #{
...
universal_provider_oauth_and_config_workflow_test() ->
    ...
    {ok, Callback} = beam_agent_provider:oauth_callback(Session, <<"openai">>, #{
        request_id => RequestId,
        code => <<"authorized">>
    }),
...
universal_config_import_and_key_write_fallbacks_test() ->
    ...
    {ok, _} = beam_agent_config:value_write(Session, <<"runtime.provider_id">>, <<"openai">>),
    {ok, _} = beam_agent_config:batch_write(Session, [
        #{key_path => <<"control.permission_mode">>, value => <<"acceptEdits">>},
        #{key_path => <<"control.max_thinking_tokens">>, value => 42}
    ]),
    {ok, _Imported} = beam_agent_config:external_agent_import(Session, #{
        config => #{
            runtime => #{agent => <<"planner">>},
            control => #{permission_mode => <<"bypassPermissions">>}
        }
    }),
    {ok, Detect1} = beam_agent_config:external_agent_detect(Session),
...
universal_thread_admin_native_leak_fallbacks_test() ->
    ...
    {ok, Compacted} =
        beam_agent_threads:thread_compact(Session, #{thread_id => ThreadId, count => 1}),
    CompactedThread = maps:get(thread, Compacted),
    ?assertEqual(1, maps:get(visible_message_count, CompactedThread)),
    {ok, Unsubscribed} = beam_agent_threads:thread_unsubscribe(Session, ThreadId),
    ...
    ?assertEqual({ok, #{
        ...
        source => universal,
        backend => gemini
    }}, beam_agent_control:turn_interrupt(Session, ThreadId, <<"turn-1">>)),
    cleanup_session(Session).
...
universal_status_and_session_admin_native_leak_fallbacks_test() ->
    ...
    {ok, Skills} = beam_agent_skills:remote_list(Session),
    ...
    {ok, Status} = beam_agent_runtime:get_status(Session),
    ...
    {ok, AuthStatus} = beam_agent_runtime:get_auth_status(Session),
    ?assertEqual(universal, maps:get(source, AuthStatus)),
    ?assertEqual(undefined, maps:get(provider_id, AuthStatus)),
    ?assertEqual({ok, SessionId}, beam_agent_runtime:get_last_session_id(Session)),
    ...
    ?assertMatch({ok, #{
        ...
        source := universal,
        backend := gemini
    }}, beam_agent_command:session_destroy(Session)),
    ?assertEqual({error, not_found}, beam_agent_session_store:get_session(SessionId)),
    cleanup_session(Session).
...
universal_command_and_turn_response_fallbacks_test() ->
    ...
    Responded = beam_agent_command:turn_respond(Session, RequestId, #{answer => <<"approved">>}),
