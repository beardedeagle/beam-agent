%%%-------------------------------------------------------------------
%%% @doc Public API tests for audit functions in beam_agent_journal.
%%%-------------------------------------------------------------------
...
public_audit_roundtrip_test() ->
    ...
    {ok, [Listed]} = beam_agent_journal:list_events(#{run_id => <<"public-audit-run">>}),
    ?assertEqual(EventId, maps:get(event_id, Listed)),
    {ok, Stored} = beam_agent_journal:get_event(EventId),
