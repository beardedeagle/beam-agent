...
ensure_tables_idempotent_test() ->
    ...
    beam_agent_runtime:app_clear(),
    ok = beam_agent_runtime:app_ensure_tables(),
    ok = beam_agent_runtime:app_ensure_tables(),
    ok = beam_agent_runtime:app_ensure_tables(),
    beam_agent_runtime:app_clear().
...
clear_removes_all_data_test() ->
    ...
    beam_agent_runtime:app_clear(),
    Sess = <<"sess-clear">>,
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"app1">>, #{}),
    {ok, [_]} = beam_agent_runtime:apps_list_impl(Sess),
    ok = beam_agent_runtime:app_clear(),
    {ok, []} = beam_agent_runtime:apps_list_impl(Sess),
    beam_agent_runtime:app_clear().
...
register_app_creates_entry_test() ->
    ...
    beam_agent_runtime:app_clear(),
    Sess = <<"sess-reg-1">>,
    {ok, Entry} = beam_agent_runtime:app_register(Sess, <<"myapp">>, #{}),
...
register_app_with_custom_name_test() ->
    beam_agent_runtime:app_ensure_tables(),
    beam_agent_runtime:app_clear(),
    {ok, Entry} = beam_agent_runtime:app_register(
...
register_app_update_existing_test() ->
    ...
    beam_agent_runtime:app_clear(),
    Sess = <<"sess-reg-upd">>,
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"app5">>, #{name => <<"v1">>}),
    {ok, Updated} = beam_agent_runtime:app_register(Sess, <<"app5">>, #{name => <<"v2">>}),
    ?assertEqual(<<"v2">>, maps:get(name, Updated)),
    %% Only one app should exist
    {ok, List} = beam_agent_runtime:apps_list_impl(Sess),
...
apps_list_empty_test() ->
    beam_agent_runtime:app_ensure_tables(),
    beam_agent_runtime:app_clear(),
    {ok, List} = beam_agent_runtime:apps_list_impl(<<"sess-empty">>),
    ?assertEqual([], List).
...
apps_list_returns_registered_test() ->
    ...
    beam_agent_runtime:app_clear(),
    Sess = <<"sess-list-1">>,
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"a1">>, #{}),
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"a2">>, #{}),
    {ok, List} = beam_agent_runtime:apps_list_impl(Sess),
...
apps_list_session_isolation_test() ->
    beam_agent_runtime:app_ensure_tables(),
    beam_agent_runtime:app_clear(),
    {ok, _} = beam_agent_runtime:app_register(<<"sessA">>, <<"a1">>, #{}),
    {ok, _} = beam_agent_runtime:app_register(<<"sessB">>, <<"b1">>, #{}),
    {ok, ListA} = beam_agent_runtime:apps_list_impl(<<"sessA">>),
    {ok, ListB} = beam_agent_runtime:apps_list_impl(<<"sessB">>),
...
apps_list_status_filter_active_test() ->
    ...
    beam_agent_runtime:app_clear(),
    Sess = <<"sess-filter">>,
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"act">>, #{status => active}),
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"inact">>, #{status => inactive}),
    {ok, Active} = beam_agent_runtime:apps_list_impl(Sess, #{status => active}),
...
apps_list_status_filter_inactive_test() ->
    ...
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"act">>, #{status => active}),
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"inact">>, #{status => inactive}),
    {ok, Inactive} = beam_agent_runtime:apps_list_impl(Sess, #{status => inactive}),
...
apps_list_no_filter_returns_all_test() ->
    ...
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"act">>, #{status => active}),
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"inact">>, #{status => inactive}),
    {ok, All} = beam_agent_runtime:apps_list_impl(Sess, #{}),
...
app_info_no_app_test() ->
    beam_agent_runtime:app_ensure_tables(),
    beam_agent_runtime:app_clear(),
    ?assertEqual({error, no_app}, beam_agent_runtime:app_info_impl(<<"sess-noapp">>)).

app_info_returns_entry_test() ->
    beam_agent_runtime:app_ensure_tables(),
    beam_agent_runtime:app_clear(),
    Sess = <<"sess-info-1">>,
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"infoapp">>, #{}),
    {ok, Entry} = beam_agent_runtime:app_info_impl(Sess),
...
app_info_returns_latest_when_multiple_test() ->
    ...
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"first">>, #{}),
    timer:sleep(5),
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"second">>, #{}),
    {ok, Entry} = beam_agent_runtime:app_info_impl(Sess),
...
app_init_creates_default_test() ->
    ...
    beam_agent_runtime:app_clear(),
    Sess = <<"sess-init-1">>,
    {ok, Entry} = beam_agent_runtime:app_init_impl(Sess),
...
app_init_idempotent_returns_existing_test() ->
    ...
    beam_agent_runtime:app_clear(),
    Sess = <<"sess-init-2">>,
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"existing">>, #{name => <<"MyApp">>}),
    {ok, Entry} = beam_agent_runtime:app_init_impl(Sess),
...
app_init_twice_same_result_test() ->
    ...
    beam_agent_runtime:app_clear(),
    Sess = <<"sess-init-3">>,
    {ok, First} = beam_agent_runtime:app_init_impl(Sess),
    {ok, Second} = beam_agent_runtime:app_init_impl(Sess),
...
app_log_appends_entry_test() ->
    ...
    beam_agent_runtime:app_clear(),
    Sess = <<"sess-log-1">>,
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"logapp">>, #{}),
    ok = beam_agent_runtime:app_log_impl(Sess, <<"first log">>),
    ok = beam_agent_runtime:app_log_impl(Sess, <<"second log">>),
    {ok, Entry} = beam_agent_runtime:app_info_impl(Sess),
...
app_log_no_app_returns_error_test() ->
    beam_agent_runtime:app_ensure_tables(),
    beam_agent_runtime:app_clear(),
    ?assertEqual({error, no_app}, beam_agent_runtime:app_log_impl(<<"sess-nolog">>, <<"msg">>)).

app_log_arbitrary_term_test() ->
    beam_agent_runtime:app_ensure_tables(),
    beam_agent_runtime:app_clear(),
    Sess = <<"sess-log-term">>,
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"termapp">>, #{}),
    ok = beam_agent_runtime:app_log_impl(Sess, #{level => error, msg => <<"boom">>}),
    {ok, Entry} = beam_agent_runtime:app_info_impl(Sess),
...
app_modes_default_when_no_app_test() ->
    beam_agent_runtime:app_ensure_tables(),
    beam_agent_runtime:app_clear(),
    {ok, Modes} = beam_agent_runtime:app_modes_impl(<<"sess-nomodes">>),
...
app_modes_default_after_register_test() ->
    ...
    beam_agent_runtime:app_clear(),
    Sess = <<"sess-modes-1">>,
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"modeapp">>, #{}),
    {ok, Modes} = beam_agent_runtime:app_modes_impl(Sess),
    ?assertEqual([<<"default">>, <<"debug">>, <<"verbose">>], Modes),
    beam_agent_runtime:app_clear().
...
app_modes_custom_after_register_test() ->
    ...
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"modeapp2">>, #{modes => Custom}),
    {ok, Modes} = beam_agent_runtime:app_modes_impl(Sess),
...
unregister_app_removes_entry_test() ->
    ...
    beam_agent_runtime:app_clear(),
    Sess = <<"sess-unreg-1">>,
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"gone">>, #{}),
    {ok, [_]} = beam_agent_runtime:apps_list_impl(Sess),
    ok = beam_agent_runtime:app_unregister(Sess, <<"gone">>),
    {ok, []} = beam_agent_runtime:apps_list_impl(Sess),
    beam_agent_runtime:app_clear().
...
unregister_app_idempotent_on_missing_test() ->
    beam_agent_runtime:app_ensure_tables(),
    beam_agent_runtime:app_clear(),
    ok = beam_agent_runtime:app_unregister(<<"sess-unreg-2">>, <<"nosuch">>),
    beam_agent_runtime:app_clear().
...
unregister_app_only_target_test() ->
    ...
    beam_agent_runtime:app_clear(),
    Sess = <<"sess-unreg-3">>,
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"keep">>, #{}),
    {ok, _} = beam_agent_runtime:app_register(Sess, <<"remove">>, #{}),
    ok = beam_agent_runtime:app_unregister(Sess, <<"remove">>),
    {ok, Remaining} = beam_agent_runtime:apps_list_impl(Sess),
