...
%%%
%%% Tests cover classify_message/2 for connection-level transport events,
%%% send/2 header construction, and close/1 DELETE logic.
%%%
%%% The `test_mcp_http_client` fixture is used for all tests that require a
%%% client connection (start/1, send/2, close/1).  For pure
%%% classify_message/2 tests, `self()` serves as ConnPid so the
%%% pattern-match succeeds without any network activity.
%%%
%%% HTTP-level messages (http_response, http_data) are verified to
...
%% This lets classify_message/2 patterns match without a real connection.
make_ref_self() ->
    ConnPid  = self(),
    MonRef   = make_ref(),
    SessState = #{path => <<"/mcp">>,
...
start_creates_proper_ref_test() ->
    ...
    try
        ...
        ?assertMatch({_ConnPid, _MonRef, test_mcp_http_client,
                      #{path := <<"/mcp">>,
                        session_id := undefined,
                        protocol_version := undefined}},
                     Ref)
...
classify_transport_up_returns_connected_test() ->
    {ConnPid, _, _, _} = Ref = make_ref_self(),
    ?assertEqual(connected,
        beam_agent_mcp_transport_http:classify_message(
            {transport_up, ConnPid, http}, Ref)).

classify_transport_down_returns_disconnected_test() ->
    {ConnPid, _, _, _} = Ref = make_ref_self(),
    ?assertEqual({disconnected, closed},
        beam_agent_mcp_transport_http:classify_message(
            {transport_down, ConnPid, closed}, Ref)).
...
classify_http_response_returns_ignore_test() ->
    {ConnPid, _, _, _} = Ref = make_ref_self(),
    StreamRef = make_ref(),
    ?assertEqual(ignore,
        beam_agent_mcp_transport_http:classify_message(
            {http_response, ConnPid, StreamRef, nofin, 200,
             [{<<"content-type">>, <<"application/json">>}]}, Ref)).

classify_http_data_returns_ignore_test() ->
    {ConnPid, _, _, _} = Ref = make_ref_self(),
    StreamRef = make_ref(),
    ?assertEqual(ignore,
        beam_agent_mcp_transport_http:classify_message(
            {http_data, ConnPid, StreamRef, fin,
             <<"{\"jsonrpc\":\"2.0\"}">>}, Ref)).

classify_http_response_202_returns_ignore_test() ->
    {ConnPid, _, _, _} = Ref = make_ref_self(),
    StreamRef = make_ref(),
    ?assertEqual(ignore,
        beam_agent_mcp_transport_http:classify_message(
            {http_response, ConnPid, StreamRef, fin, 202, []}, Ref)).

classify_http_response_404_returns_ignore_test() ->
    {ConnPid, _, _, _} = Ref = make_ref_self(),
    StreamRef = make_ref(),
    ?assertEqual(ignore,
        beam_agent_mcp_transport_http:classify_message(
            {http_response, ConnPid, StreamRef, nofin, 404, []}, Ref)).
...
classify_wrong_conn_pid_returns_ignore_test() ->
    ...
    ?assertEqual(ignore,
        beam_agent_mcp_transport_http:classify_message(
            {transport_up, OtherPid, http}, Ref)).
...
send_posts_to_mcp_endpoint_test() ->
    ...
    try
        ...
        receive
            {http_request, post, <<"/mcp">>, _Headers, _Body, _StreamRef} ->
                ok
        after 200 ->
...
send_includes_content_type_header_test() ->
    ...
    try
        ...
        receive
            {http_request, post, _, Headers, _, _} ->
                ?assert(lists:member(
...
send_includes_session_id_when_set_test() ->
    test_mcp_http_client:setup(),
    try
        {ok, {ConnPid, MonRef, ClientMod, SessState}} =
        ...
        RefWithSid = {ConnPid, MonRef, ClientMod,
...
send_includes_protocol_version_when_set_test() ->
    ...
    try
        ...
        RefWithVer = {ConnPid, MonRef, ClientMod,
...
close_sends_delete_when_session_active_test() ->
    ...
    try
        ...
        receive
            {http_request, delete, <<"/mcp">>, Headers, _StreamRef} ->
                ?assert(lists:member(
...
is_ready_test() ->
    ...
    try
        {ok, Ref} = beam_agent_mcp_transport_http:start(#{
            client_module => test_mcp_http_client, host => <<"localhost">>,
            port => 4096, path => <<"/mcp">>}),
