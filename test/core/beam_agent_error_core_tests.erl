...
categorize_passthrough_existing_category_test() ->
    ...
    ?assertEqual(Msg, beam_agent_error_core:categorize(Msg)).

categorize_infers_from_content_test() ->
    Msg = #{type => error, content => <<"rate limit exceeded">>},
    Result = beam_agent_error_core:categorize(Msg),
...
infer_rate_limit_text_test() ->
    ?assertEqual(rate_limit, beam_agent_error_core:infer_category(<<"rate limit exceeded">>)).

infer_rate_limit_underscore_test() ->
    ?assertEqual(rate_limit, beam_agent_error_core:infer_category(<<"rate_limit">>)).

infer_rate_limit_joined_test() ->
    ?assertEqual(rate_limit, beam_agent_error_core:infer_category(<<"ratelimit">>)).

infer_rate_limit_too_many_requests_test() ->
    ?assertEqual(rate_limit, beam_agent_error_core:infer_category(<<"too many requests">>)).

infer_rate_limit_429_test() ->
    ?assertEqual(rate_limit, beam_agent_error_core:infer_category(<<"HTTP 429 error">>)).

infer_rate_limit_throttled_test() ->
    ?assertEqual(rate_limit, beam_agent_error_core:infer_category(<<"request throttled">>)).

infer_rate_limit_throttling_test() ->
    ?assertEqual(rate_limit, beam_agent_error_core:infer_category(<<"throttling applied">>)).
...
infer_subscription_quota_exceeded_test() ->
    ?assertEqual(subscription_exhausted,
        beam_agent_error_core:infer_category(<<"quota exceeded">>)).

infer_subscription_quota_underscore_test() ->
    ?assertEqual(subscription_exhausted,
        beam_agent_error_core:infer_category(<<"quota_exceeded for account">>)).

infer_subscription_usage_cap_test() ->
    ?assertEqual(subscription_exhausted,
        beam_agent_error_core:infer_category(<<"usage cap reached">>)).

infer_subscription_usage_limit_test() ->
    ?assertEqual(subscription_exhausted,
        beam_agent_error_core:infer_category(<<"usage limit hit">>)).

infer_subscription_plan_limit_test() ->
    ?assertEqual(subscription_exhausted,
        beam_agent_error_core:infer_category(<<"plan limit exceeded">>)).

infer_subscription_billing_test() ->
    ?assertEqual(subscription_exhausted,
        beam_agent_error_core:infer_category(<<"billing issue detected">>)).

infer_subscription_credits_exhausted_test() ->
    ?assertEqual(subscription_exhausted,
        beam_agent_error_core:infer_category(<<"credits exhausted">>)).

infer_subscription_tokens_exhausted_test() ->
    ?assertEqual(subscription_exhausted,
        beam_agent_error_core:infer_category(<<"tokens exhausted">>)).
...
infer_context_length_test() ->
    ?assertEqual(context_exceeded,
        beam_agent_error_core:infer_category(<<"context length exceeded">>)).

infer_context_window_test() ->
    ?assertEqual(context_exceeded,
        beam_agent_error_core:infer_category(<<"context window full">>)).

infer_context_underscore_length_test() ->
    ?assertEqual(context_exceeded,
        beam_agent_error_core:infer_category(<<"context_length overflow">>)).

infer_context_underscore_window_test() ->
    ?assertEqual(context_exceeded,
        beam_agent_error_core:infer_category(<<"context_window exceeded">>)).

infer_context_token_limit_test() ->
    ?assertEqual(context_exceeded,
        beam_agent_error_core:infer_category(<<"token limit reached">>)).

infer_context_too_many_tokens_test() ->
    ?assertEqual(context_exceeded,
        beam_agent_error_core:infer_category(<<"too many tokens in request">>)).

infer_context_maximum_context_test() ->
    ?assertEqual(context_exceeded,
        beam_agent_error_core:infer_category(<<"maximum context reached">>)).

infer_context_max_tokens_test() ->
    ?assertEqual(context_exceeded,
        beam_agent_error_core:infer_category(<<"max_tokens exceeded">>)).

infer_context_input_too_long_test() ->
    ?assertEqual(context_exceeded,
        beam_agent_error_core:infer_category(<<"input too long for model">>)).
...
infer_auth_unauthorized_test() ->
    ?assertEqual(auth_expired,
        beam_agent_error_core:infer_category(<<"unauthorized access">>)).

infer_auth_unauthenticated_test() ->
    ?assertEqual(auth_expired,
        beam_agent_error_core:infer_category(<<"unauthenticated request">>)).

infer_auth_invalid_api_key_test() ->
    ?assertEqual(auth_expired,
        beam_agent_error_core:infer_category(<<"invalid api key">>)).

infer_auth_invalid_api_key_underscore_test() ->
    ?assertEqual(auth_expired,
        beam_agent_error_core:infer_category(<<"invalid_api_key">>)).

infer_auth_api_key_expired_test() ->
    ?assertEqual(auth_expired,
        beam_agent_error_core:infer_category(<<"api key expired">>)).

infer_auth_token_expired_test() ->
    ?assertEqual(auth_expired,
        beam_agent_error_core:infer_category(<<"token expired">>)).

infer_auth_access_denied_test() ->
    ?assertEqual(auth_expired,
        beam_agent_error_core:infer_category(<<"access denied">>)).

infer_auth_forbidden_test() ->
    ?assertEqual(auth_expired,
        beam_agent_error_core:infer_category(<<"forbidden resource">>)).

infer_auth_401_test() ->
    ?assertEqual(auth_expired,
        beam_agent_error_core:infer_category(<<"401 Unauthorized">>)).

infer_auth_403_test() ->
    ?assertEqual(auth_expired,
        beam_agent_error_core:infer_category(<<"403 Forbidden">>)).
...
infer_server_internal_server_error_test() ->
    ?assertEqual(server_error,
        beam_agent_error_core:infer_category(<<"internal server error">>)).

infer_server_server_error_test() ->
    ?assertEqual(server_error,
        beam_agent_error_core:infer_category(<<"server error occurred">>)).

infer_server_service_unavailable_test() ->
    ?assertEqual(server_error,
        beam_agent_error_core:infer_category(<<"service unavailable">>)).

infer_server_bad_gateway_test() ->
    ?assertEqual(server_error,
        beam_agent_error_core:infer_category(<<"bad gateway">>)).

infer_server_gateway_timeout_test() ->
    ?assertEqual(server_error,
        beam_agent_error_core:infer_category(<<"gateway timeout">>)).

infer_server_overloaded_test() ->
    ?assertEqual(server_error,
        beam_agent_error_core:infer_category(<<"server overloaded">>)).

infer_server_500_test() ->
    ?assertEqual(server_error,
        beam_agent_error_core:infer_category(<<"500 Internal Server Error">>)).

infer_server_502_test() ->
    ?assertEqual(server_error,
        beam_agent_error_core:infer_category(<<"502 Bad Gateway">>)).

infer_server_503_test() ->
    ?assertEqual(server_error,
        beam_agent_error_core:infer_category(<<"503 Service Unavailable">>)).

infer_server_504_test() ->
    ?assertEqual(server_error,
        beam_agent_error_core:infer_category(<<"504 Gateway Timeout">>)).
...
infer_unknown_for_unrecognized_test() ->
    ?assertEqual(unknown, beam_agent_error_core:infer_category(<<"something else">>)).

infer_unknown_for_empty_binary_test() ->
    ?assertEqual(unknown, beam_agent_error_core:infer_category(<<>>)).

infer_unknown_for_non_binary_test() ->
    ?assertEqual(unknown, beam_agent_error_core:infer_category(12345)).

infer_unknown_for_atom_test() ->
    ?assertEqual(unknown, beam_agent_error_core:infer_category(some_atom)).

infer_unknown_for_list_test() ->
    ?assertEqual(unknown, beam_agent_error_core:infer_category("a string list")).
...
infer_priority_rate_limit_over_server_test() ->
    ?assertEqual(rate_limit,
        beam_agent_error_core:infer_category(<<"rate limit: 429 server error">>)).
...
enrich_extracts_binary_category_test() ->
    ...
    Result = beam_agent_error_core:enrich(Msg, Raw),
    ?assertEqual(rate_limit, maps:get(category, Result)).
...
enrich_passes_non_error_unchanged_test() ->
    Msg = #{type => text, content => <<"hello">>},
    Raw = #{<<"category">> => <<"rate_limit">>},
    ?assertEqual(Msg, beam_agent_error_core:enrich(Msg, Raw)).
...
parse_retry_after_positive_integer_test() ->
    ?assertEqual(30, beam_agent_error_core:parse_retry_after(30)).

parse_retry_after_binary_digits_test() ->
    ?assertEqual(45, beam_agent_error_core:parse_retry_after(<<"45">>)).

parse_retry_after_float_test() ->
    ?assertEqual(3, beam_agent_error_core:parse_retry_after(2.5)).

parse_retry_after_float_ceil_test() ->
    ?assertEqual(10, beam_agent_error_core:parse_retry_after(9.1)).

parse_retry_after_zero_integer_test() ->
    ?assertEqual(undefined, beam_agent_error_core:parse_retry_after(0)).

parse_retry_after_negative_integer_test() ->
    ?assertEqual(undefined, beam_agent_error_core:parse_retry_after(-10)).

parse_retry_after_zero_binary_test() ->
    ?assertEqual(undefined, beam_agent_error_core:parse_retry_after(<<"0">>)).

parse_retry_after_negative_binary_test() ->
    ?assertEqual(undefined, beam_agent_error_core:parse_retry_after(<<"-5">>)).

parse_retry_after_non_numeric_binary_test() ->
    ?assertEqual(undefined, beam_agent_error_core:parse_retry_after(<<"abc">>)).

parse_retry_after_undefined_test() ->
    ?assertEqual(undefined, beam_agent_error_core:parse_retry_after(undefined)).

parse_retry_after_atom_test() ->
    ?assertEqual(undefined, beam_agent_error_core:parse_retry_after(none)).

parse_retry_after_zero_float_test() ->
    ?assertEqual(undefined, beam_agent_error_core:parse_retry_after(0.0)).

parse_retry_after_negative_float_test() ->
    ?assertEqual(undefined, beam_agent_error_core:parse_retry_after(-1.5)).

parse_retry_after_large_integer_test() ->
    ?assertEqual(3600, beam_agent_error_core:parse_retry_after(3600)).
