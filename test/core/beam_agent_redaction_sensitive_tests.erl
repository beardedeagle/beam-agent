%%%-------------------------------------------------------------------
%%% @doc EUnit tests for sensitive key functions in beam_agent_redaction.
%%%
%%% Tests cover:
...
%%% @end
%%%-------------------------------------------------------------------
-module(beam_agent_redaction_sensitive_tests).
...
all_returns_18_entries_test() ->
    ?assertEqual(18, length(beam_agent_redaction:all())).
...
all_entries_have_valid_types_test() ->
    ...
    end, beam_agent_redaction:all()).

no_duplicate_canonical_names_test() ->
    Names = [Name || {Name, _, _} <- beam_agent_redaction:all()],
    ?assertEqual(length(Names), length(lists:usort(Names))).

nine_encrypt_and_redact_entries_test() ->
    EncryptKeys = [Name || {Name, _, encrypt_and_redact} <- beam_agent_redaction:all()],
...
nine_redact_only_entries_test() ->
    RedactKeys = [Name || {Name, _, redact_only} <- beam_agent_redaction:all()],
...
credential_keys_excludes_redact_only_test() ->
    CredKeys = beam_agent_redaction:credential_match_keys(),
    %% None of the redact_only canonical atoms should appear
    RedactOnlyAtoms = [Name || {Name, _, redact_only} <- beam_agent_redaction:all()],
...
redaction_keys_contains_all_18_canonical_forms_test() ->
    RedactKeys = beam_agent_redaction:redaction_match_keys(),
...
every_credential_key_appears_in_redaction_test() ->
    CredNames = [Name || {Name, _, encrypt_and_redact} <- beam_agent_redaction:all()],
    RedactKeys = beam_agent_redaction:redaction_match_keys(),
    lists:foreach(fun(Name) ->
        Canonical = beam_agent_redaction:canonical_form(Name),
...
is_sensitive_atom_key_test() ->
    ?assert(beam_agent_redaction:is_sensitive(api_key)),
    ?assert(beam_agent_redaction:is_sensitive(token)),
    ?assert(beam_agent_redaction:is_sensitive(password)).

is_sensitive_camel_case_binary_test() ->
    ?assert(beam_agent_redaction:is_sensitive(<<"apiKey">>)),
    ?assert(beam_agent_redaction:is_sensitive(<<"accessToken">>)),
    ?assert(beam_agent_redaction:is_sensitive(<<"refreshToken">>)).

is_sensitive_snake_case_binary_test() ->
    ?assert(beam_agent_redaction:is_sensitive(<<"api_key">>)),
    ?assert(beam_agent_redaction:is_sensitive(<<"access_token">>)),
    ?assert(beam_agent_redaction:is_sensitive(<<"private_key">>)).

is_sensitive_upper_case_binary_test() ->
    ?assert(beam_agent_redaction:is_sensitive(<<"API_KEY">>)),
    ?assert(beam_agent_redaction:is_sensitive(<<"GITHUB_TOKEN">>)),
    ?assert(beam_agent_redaction:is_sensitive(<<"PASSWORD">>)).

is_sensitive_redact_only_keys_test() ->
    ?assert(beam_agent_redaction:is_sensitive(authorization)),
    ?assert(beam_agent_redaction:is_sensitive(<<"bearerToken">>)),
    ?assert(beam_agent_redaction:is_sensitive(<<"session_token">>)),
    ?assert(beam_agent_redaction:is_sensitive(<<"CREDENTIAL_KEY">>)).

is_sensitive_non_sensitive_returns_false_test() ->
    ?assertNot(beam_agent_redaction:is_sensitive(username)),
    ?assertNot(beam_agent_redaction:is_sensitive(<<"model">>)),
    ?assertNot(beam_agent_redaction:is_sensitive(<<"baseUrl">>)),
    ?assertNot(beam_agent_redaction:is_sensitive(<<"ORGANIZATION">>)).
...
snake_to_camel_multi_word_test() ->
    ?assertEqual(<<"apiKey">>, beam_agent_redaction:snake_to_camel(<<"api_key">>)),
    ?assertEqual(<<"accessToken">>, beam_agent_redaction:snake_to_camel(<<"access_token">>)),
    ?assertEqual(<<"clientSecret">>, beam_agent_redaction:snake_to_camel(<<"client_secret">>)),
    ?assertEqual(<<"githubToken">>, beam_agent_redaction:snake_to_camel(<<"github_token">>)).

snake_to_camel_single_word_test() ->
    ?assertEqual(<<"token">>, beam_agent_redaction:snake_to_camel(<<"token">>)),
    ?assertEqual(<<"secret">>, beam_agent_redaction:snake_to_camel(<<"secret">>)).
...
canonical_form_strips_underscores_test() ->
    ?assertEqual(<<"apikey">>, beam_agent_redaction:canonical_form(api_key)),
    ?assertEqual(<<"accesstoken">>, beam_agent_redaction:canonical_form(access_token)),
    ?assertEqual(<<"authorizationcode">>, beam_agent_redaction:canonical_form(authorization_code)).

canonical_form_single_word_unchanged_test() ->
    ?assertEqual(<<"token">>, beam_agent_redaction:canonical_form(token)),
    ?assertEqual(<<"secret">>, beam_agent_redaction:canonical_form(secret)),
    ?assertEqual(<<"password">>, beam_agent_redaction:canonical_form(password)).
...
normalize_key_strips_and_lowercases_test() ->
    ?assertEqual(<<"apikey">>, beam_agent_redaction:normalize_key(<<"API_KEY">>)),
    ?assertEqual(<<"apikey">>, beam_agent_redaction:normalize_key(<<"apiKey">>)),
    ?assertEqual(<<"apikey">>, beam_agent_redaction:normalize_key(<<"api_key">>)),
    ?assertEqual(<<"apikey">>, beam_agent_redaction:normalize_key(<<"Api-Key">>)).
