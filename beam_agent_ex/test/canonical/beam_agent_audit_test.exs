defmodule BeamAgent.AuditTest do
  ...
  test "journal exports the canonical audit convenience accessors" do
  ...
  test "lists and fetches audit events via Journal" do
    assert {:ok, event} =
    ...
    assert {:ok, [listed]} = BeamAgent.Journal.list_events(%{run_id: "ex-audit-run"})
    assert (listed[:event_id] || listed["event_id"]) == event_id
    assert {:ok, stored} = BeamAgent.Journal.get_event(event_id)
