...
-moduledoc """
Public API for canonical BeamAgent backend routing and session lifecycle.

This module serves two purposes:

1. **Policy Routing** — select a backend according to reusable policy instead of
   hard-coding one every time.

2. **Session Router** — start, stop, query, and manage unified sessions after
   resolving the backend via routing policy. This is the main entry point for
   session lifecycle operations used by `beam_agent_core`.
