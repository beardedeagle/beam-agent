...
This module delegates to `beam_agent_registry` (kind = `agent`) for all
operations. Agent types are registered globally and shared across all
...
register(Id, Opts) -> beam_agent_registry:register(agent, Id, Opts).

-doc "Unregister an agent type by id. Idempotent.".
-spec unregister(binary()) -> ok.
unregister(Id) -> beam_agent_registry:unregister(agent, Id).

-doc "Fetch a single agent type by id.".
-spec get(binary()) ->
    {ok, beam_agent_registry:agent_def()} | {error, not_found}.
get(Id) -> beam_agent_registry:get(agent, Id).

-doc "List all registered agent types.".
-spec list() -> [beam_agent_registry:agent_def()].
list() -> beam_agent_registry:list(agent).

-doc "Remove all registered agent types.".
-spec clear() -> ok.
clear() -> beam_agent_registry:clear(agent).
