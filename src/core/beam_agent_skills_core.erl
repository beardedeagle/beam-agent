...
-moduledoc """
Universal skills management for the BEAM Agent SDK.

Provides ETS-backed skill registration, listing, remote discovery,
export, and config management. Acts as the universal fallback for
skill-related operations when a backend does not expose native skills
management APIs.

The module maintains two key namespaces per session in the
`beam_agent_skills` ETS table:

  - Skills: `{SessionKey, skill, SkillId}` — registered skill entries.
  - Config: `{SessionKey, config, Path}` — per-path skill configuration.

`SessionKey` is either the raw pid or a binary session identifier.

Usage:
```erlang
%% Ensure the backing table exists (idempotent):
beam_agent_skills_core:ensure_table(),

%% Register a skill:
{ok, Entry} = beam_agent_skills_core:register_skill(
    self(),
    <<"my-skill">>,
    #{name => <<"My Skill">>,
...
%% List all skills for a session:
{ok, Skills} = beam_agent_skills_core:skills_list(self()),

%% Write config:
ok = beam_agent_skills_core:skills_config_write(self(), <<"my-skill">>, true),

%% Read config:
{ok, Configs} = beam_agent_skills_core:skills_config_read(self())
```
""".
