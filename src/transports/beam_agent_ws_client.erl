...
-moduledoc """
RFC 6455 WebSocket client for beam_agent transports.

Manages a gen_tcp/ssl connection through the WebSocket lifecycle:
TCP connect → HTTP upgrade handshake → frame send/receive → close.

Uses `beam_agent_ws_frame` for pure frame encoding/decoding.

## Connection Lifecycle

    open/3 → TCP/TLS connect → {transport_up, Pid, http}
    ws_upgrade/3 → HTTP upgrade → {ws_upgraded, Pid, Ref, ...}
    ws_send/3 → masked frame over socket
    receive frame → {ws_frame, Pid, Ref, Frame}
    close/1 → close handshake → socket close

## Message Protocol

The owner process receives:

  - `{transport_up, Pid, http}` — TCP/TLS connected
  - `{ws_upgraded, Pid, Ref, [<<\"websocket\">>], Headers}` — upgrade done
  - `{ws_frame, Pid, Ref, {text, Payload}}` — text frame
  - `{ws_frame, Pid, Ref, {binary, Payload}}` — binary frame
  - `{ws_frame, Pid, Ref, {close, Code, Reason}}` — close frame
  - `{transport_down, Pid, Reason}` — connection lost

Ping frames are handled automatically with pong responses.

## DI Interface

Implements the same function signatures as `beam_agent_transport_ws`
expects:

  - `open/3` — connect TCP/TLS
  - `ws_upgrade/3` — perform HTTP upgrade handshake
  - `ws_send/3` — send a WebSocket frame
  - `close/1` — close the connection

## Security

  - TLS via OTP `ssl` module with `verify_peer` by default
  - Client frames masked with `crypto:strong_rand_bytes/1` keys
  - Sec-WebSocket-Accept validated per RFC 6455 Section 4.2.2
  - Frame size limits enforced (default 64 MB)
""".
